const string Program_Name = "Scouting"; //Name me!
...
struct Coord{
	...
	public string Latitude_Str(){
		string output="";
		int number=(int)Math.Abs(Latitude);
		output+=number.ToString()+"°";
		number=(int)Math.Abs(Latitude*60)%60;
		output+=number.ToString()+"'";
		number=(int)Math.Abs(Latitude*60*60)%60;
		output+=number.ToString()+'"';
		if(Latitude<0)
			output+="S";
		else
	...
	public string Longitude_Str(){
		string output="";
		int number=(int)Math.Abs(Longitude);
		output+=number.ToString()+"°";
		number=(int)Math.Abs(Longitude*60)%60;
		output+=number.ToString()+"'";
		number=(int)Math.Abs(Longitude*60*60)%60;
		output+=number.ToString()+'"';
		if(Longitude<0)
			output+="W";
		else
	...
	public string Elevation_Str(int round=1){
		if(Math.Abs(Elevation)>=1000){
			if(Math.Abs(Elevation)/10>299792.45800000185)
				return Math.Round(Elevation/1000/299792.45800000185,round+1).ToString()+"ls";
			else
				return Math.Round(Elevation/1000,round).ToString()+"kM";
		}
		else
			return Math.Round(Elevation,round).ToString()+"M";
	}
	
	public Coord(double la,double lo,double el){
		Latitude=la;
		Longitude=lo;
		Elevation=el;
	}
	
	public Coord(Vector3D Coords,Vector3D PlanetCenter,double Sealevel){
		Elevation=Sealevel;
		double thickness=(PlanetCenter-Coords).Length()-Elevation;
		Vector3D latitude=Coords;
		latitude.Normalize();
		Vector3D north=new Vector3D(0,-1,0);
		Latitude=90-Prog.GetAngle(north,latitude);
		
		Vector3D longitude=Coords;
		longitude.Y=0;
		longitude.Normalize();
		Vector3D east=new Vector3D(-1,0,0);
		Longitude=90-Prog.GetAngle(east,longitude);
	}
...
public void Main(string argument, UpdateType updateSource)
{
	...
	Vector3D center;
	double elevation;
	if(Controller.TryGetPlanetPosition(out center)&&Controller.TryGetPlanetElevation(MyPlanetElevation.Sealevel,out elevation)){
		Coord coords=new Coord(Controller.GetPosition(),center,elevation);
		Write("Latitude:"+coords.Latitude_Str());
		Write("Longitude:"+coords.Longitude_Str());
		Write("Elevation:"+coords.Elevation_Str());
	}
}
